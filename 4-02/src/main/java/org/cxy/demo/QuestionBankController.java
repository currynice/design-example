...
public class QuestionBankController {
    ...
    public String createPaper(String candidate, String number) throws CloneNotSupportedException {
        ...
        return print(questionBankClone);  

    }



    private String print(QuestionBank questionBank){
        // 输出结果
        StringBuilder detail = new StringBuilder("考生：" + questionBank.getCandidate() + "\r\n" +
                "考号：" + questionBank.getNumber() + "\r\n" +
                "--------------------------------------------\r\n" +
                "一、选择题" + "\r\n\n");

        List<ChoiceQuestion> choiceQuestionList = questionBank.getChoiceQuestionList();

        List<AnswerQuestion> answerQuestionList = questionBank.getAnswerQuestionList();


        for (int idx = 0; idx < choiceQuestionList.size(); idx++) {
            detail.append("第").append(idx + 1).append("题：").append(choiceQuestionList.get(idx).getDesc()).append("\r\n");
        ...
        for (int idx = 0; idx < answerQuestionList.size(); idx++) {
            detail.append("第").append(idx + 1).append("题：").append(answerQuestionList.get(idx).getDesc()).append("\r\n");
